...
window.addNuevoEmpleado = async function (event) {
  ...
  const { nombre, edad, cedula, sexo, cargo, telefono, añoIngreso } = formDataJSON;

  try {
    await addEmpleado(nombre, edad, cedula, sexo, cargo, telefono, añoIngreso);
    formulario.reset();
    setTimeout(() => {
...
async function cargarDetalleEmpleado(id) {
  try {
    ...
    if (empleadoDoc.exists()) {
      ...
      const { nombre, edad, cedula, sexo, cargo, telefono, añoIngreso } = empleadoData;
...
async function getEmpleadoUpdateCollection(id) {
  try {
    ...
    if (empleadoDoc.exists()) {
      ...
      document.querySelector("#añoIngreso").value = añoIngreso;
    } else {
      console.log("No se encontró ningún empleado con el ID:", id);
    }
  } catch (error) {
...
window.actualizarEmpleado = async function (event) {
  ...
  const { idEmpleado, nombre, edad, cedula, sexo, cargo, telefono, añoIngreso} = formDataJSON;
  try {
    await updateEmpleadoCollection(idEmpleado, { nombre, edad, cedula, sexo, cargo, telefono, añoIngreso});
    formulario.reset();
    setTimeout(() => {
